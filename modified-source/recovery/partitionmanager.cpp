...
clear:
	...
	if (recovery_mode && !parse_userdata) {
		...
		if (TWFunc::Find_Fstab(Fstab_Filename)) {
			...
			TWFunc::Exec_Cmd("sed -i 's/,checkpoint=fs//g' " + additional_fstab);
		...
		if (!Decrypt_Data->Key_Directory.empty()) {
			...
			if (android::vold::fscrypt_mount_metadata_encrypted(Decrypt_Data->Actual_Block_Device, Decrypt_Data->Mount_Point, false, false, Decrypt_Data->Current_File_System, false, user_devices, device_aliased, 0, TWFunc::Path_Exists(additional_fstab) ? additional_fstab : "")) {
				std::string crypto_blkdev =
				        android::base::GetProperty("ro.crypto.fs_crypto_blkdev", "");

				if (crypto_blkdev.empty() ||
				    crypto_blkdev == "error" ||
				    !TWFunc::Path_Exists(crypto_blkdev)) {
				        const std::string userdata_mapper = "/dev/block/mapper/userdata";
				        if (TWFunc::Path_Exists(userdata_mapper)) {
				                crypto_blkdev = userdata_mapper;
				                LOGINFO("Using metadata userdata mapper fallback: '%s'\n",
				                        crypto_blkdev.c_str());
				        }
				}
