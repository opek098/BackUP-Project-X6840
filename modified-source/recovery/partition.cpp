...
void TWPartition::Partition_Post_Processing(bool Display_Error) {
        ...
        // X6840 V14: Advanced Wipe support for logical partitions
        static const std::vector<std::string> x6840_wipe_partitions = {
                "/system_root",
                "/system_ext",
                "/vendor",
                "/product",
        ...
        if (std::find(x6840_wipe_partitions.begin(),
                      x6840_wipe_partitions.end(),
                      Mount_Point) != x6840_wipe_partitions.end()) {

                Can_Be_Wiped = true;
                Wipe_Available_in_GUI = true;

                LOGINFO("X6840 V14: Advanced Wipe enabled for %s\n",
                        Mount_Point.c_str());
        }
...
void TWPartition::Check_FS_Type() {
	...
	if (Ignore_Blkid)
		return; // Skip probing when explicitly disabled for this partition
