...
int MtpFfsHandle::start(bool ptp) {
    // Reopen ep0 if a previous failed attempt closed it, and write the
    // MTP/PTP descriptors before touching the endpoints: the kernel only
    // creates the ep1/ep2 files and marks the function active after it
    // receives the descriptors, and init only binds ffs.mtp to the UDC
    // once sys.usb.ffs.mtp.ready (set by writeDescriptors) is true.
    if (mControl < 0) {
        mControl.reset(TEMP_FAILURE_RETRY(open(ptp ? FFS_PTP_EP0 : FFS_MTP_EP0, O_RDWR)));
        if (mControl < 0) {
            PLOG(ERROR) << (ptp ? FFS_PTP_EP0 : FFS_MTP_EP0) << ": cannot open control ep";
            return -1;
        }
    }

    if (!writeDescriptors(ptp))
        return -1;
