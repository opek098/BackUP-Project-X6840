...
bool fscrypt_mount_metadata_encrypted(const std::string& blk_device, const std::string& mount_point,
    ...
    LOG(INFO) << "TWRP metadata mapper device: " << crypto_blkdev;

    bool twrp_mount_ok = false;

    // TWRP fallback for metadata-encrypted userdata.
    // fs_mgr_do_mount hangs on this device, while a direct F2FS mount works.
    errno = 0;
    int direct_mount_rc =
            mount(crypto_blkdev.c_str(), mount_point.c_str(), "f2fs", 0, nullptr);

    if (direct_mount_rc == 0 || errno == EBUSY) {
        twrp_mount_ok = true;
        LOG(INFO) << "TWRP direct metadata mount SUCCESS: "
                  << crypto_blkdev << " -> " << mount_point;
    } else {
        PLOG(ERROR) << "TWRP direct metadata mount failed, trying fs_mgr";

        twrp_mount_ok =
                mount_via_fs_mgr(mount_point.c_str(), crypto_blkdev.c_str(), needs_encrypt);
    }

    LOG(INFO) << "TWRP metadata mount result: "
              << (twrp_mount_ok ? "SUCCESS" : "FAILED");
